...
const DateInput = ({ dataValue = new Date(), value, onChange }) => {
  const [date, setDate] = React.useState(() => {
    const d = value ? new Date(value) : dataValue;
    return {
      day: d.getDate(),
  ...
  return (
    <div className="flex border rounded-lg items-center text-sm px-0 w-fit">
      <Input
        ...
        className="p-0 h-[2.41rem] outline-none w-8 border-none text-center"
        placeholder="MM"
      />
      ...
      <Input
        ...
        className="p-0 h-[2.41rem] outline-none w-12 border-none text-center"
        placeholder="YYYY"
      />
    </div>
  );
